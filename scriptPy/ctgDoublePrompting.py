...
MODEL = "deepseek/deepseek-r1:free" #"deepseek/deepseek-r1"   #"meta-llama/llama-4-maverick:free"  #"anthropic/claude-3.7-sonnet" #"google/gemini-2.0-flash-thinking-exp:free"  #"mistralai/mistral-small-3.1-24b-instruct:free" #"deepseek/deepseek-chat-v3-0324:free" #"deepseek/deepseek-r1:free"
...
def estimate_co2_from_materials(product_data, materials_json, llm_model=MODEL):
    ...
    """
    prompt = f"""
    You are an expert in LCA and environmental life cycle assessment, focusing on cradle-to-gate analysis.

    From the following material composition and product information,
    estimate the CO2e emissions in kg considering ONLY these phases:
    ...
    Base your calculation on emission factors common to the listed materials
    and standard manufacturing processes up to factory gate.
    
    If the materials indicated are not available, estimate from the input product data,
    but maintain the cradle-to-gate boundary.

    Reply only in JSON with this format:
    {{
        "co2e_kg": ...,
        "explanation": "detailed explanation of cradle-to-gate emissions calculation"
    }}
...
def main(num_rows):
    ...
    with open("cradle_to_gate_deepseekR1_double.json", "w", encoding="utf-8") as out:
